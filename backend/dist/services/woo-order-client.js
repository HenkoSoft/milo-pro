...
exports.sanitizeWooOrderFilters = sanitizeWooOrderFilters;
function clampWooPerPage(value) {
    const perPage = Number(value);
    return Math.min(Math.max(Number.isFinite(perPage) ? perPage : 25, 1), 100);
}
function clampWooPage(value) {
    const page = Number(value);
    return Math.max(Number.isFinite(page) ? page : 1, 1);
}
function sanitizeWooOrderFilters(filters = {}) {
