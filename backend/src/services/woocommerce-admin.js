...
function toNullableWooString(value) {
  if (value === undefined || value === null || value === '') return null;
  return String(value).trim();
}

function toRequiredWooString(value, fallback = '') {
  return String(value ?? fallback).trim() || fallback;
}

function toWooNumber(value, fallback = 0) {
  const num = Number(value);
  return Number.isFinite(num) ? num : fallback;
}

function toWooJsonString(value, fallback) {
...
function preserveWooSecret(input, fallback = '') {
...
function sanitizeWooStatusResponse(record) {
  ...
  const logsSummary = data.logs_summary && typeof data.logs_summary === 'object' ? data.logs_summary : {};
...
function sanitizeWooPollingResult(record) {
  return {
    success: parseWooBoolean(record && record.success, false),
    error: toNullableWooString(record && record.error) || undefined,
    alreadyRunning: record && record.alreadyRunning === undefined ? undefined : parseWooBoolean(record && record.alreadyRunning, false),
    interval_seconds: record && record.interval_seconds === undefined ? undefined : toWooNumber(record && record.interval_seconds),
    message: toNullableWooString(record && record.message) || undefined
  };
}

function sanitizeWooPollingStatus(record) {
  return {
    active: parseWooBoolean(record && record.active, false),
    interval_seconds: toWooNumber(record && record.interval_seconds)
  };
}
...
function buildWooStatusResponse(config = null, options = {}) {
  ...
  const orderConfig = options.orderConfig || {};
