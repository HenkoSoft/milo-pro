...
const NodeBuffer = require('node:buffer').Buffer as { from: (input: string) => { toString: (encoding?: string) => string } };
...
type AdminDeps = {
  ...
  buildWooStatusResponse: (config: unknown, options?: Record<string, unknown>) => unknown;
  getWooOrderSyncConfig?: () => Promise<unknown> | unknown;
  getWooOrderSyncConfigAsync?: () => Promise<unknown>;
  initializeWooAutomation: () => Promise<unknown> | unknown;
  isWooPollingActive: () => boolean;
  normalizeWooConfigPayload: (body: unknown, existing: unknown) => Record<string, unknown>;
  normalizeWooConnectionTestPayload: (body: unknown) => Record<string, unknown>;
  stopWooPolling: (reason?: string) => void;
  woocommerceRequest: (method: string, path: string, data?: unknown, config?: unknown) => Promise<any>;
};

export function registerWooAdminRoutes(router: any, deps: AdminDeps) {
  const {
    authenticate,
  ...
  router.get('/status', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  ...
  router.get('/test', authenticate, async (_req: RouteRequest, res: JsonResponse) => {
    try {
  ...
  router.post('/test-connection', authenticate, async (req: RouteRequest, res: JsonResponse) => {
    if (req.user?.role !== 'admin') {
      res.status(403).json({ error: 'Solo el administrador puede probar configuraciones' });
      return;
    }

    const payload = normalizeWooConnectionTestPayload(req.body) as Record<string, unknown>;
    const { store_url, consumer_key, consumer_secret, api_version, wp_username, wp_app_password } = payload;
    if (!store_url || !consumer_key || !consumer_secret) {
      res.status(400).json({ error: 'Complete URL, Consumer Key y Consumer Secret.' });
      return;
    }
    ...
    try {
      ...
      if (result && result.environment) {
        if (wp_username && wp_app_password) {
          const wpUrl = new URL(String(store_url));
          const transport = require(wpUrl.protocol === 'http:' ? 'http' : 'https');
          const auth = NodeBuffer.from(`${wp_username}:${wp_app_password}`).toString('base64');
          await new Promise((resolve, reject) => {
            const reqWp = transport.request({
              ...
              let body = '';
              resWp.on('data', (chunk: unknown) => { body += String(chunk ?? ''); });
              resWp.on('end', () => {
  ...
  router.delete('/disconnect', authenticate, async (req: RouteRequest, res: JsonResponse) => {
    if (req.user?.role !== 'admin') {
      res.status(403).json({ error: 'Solo el administrador puede modificar configuraciones' });
      return;
    }
