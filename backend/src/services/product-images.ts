...
const NodeBuffer = require('node:buffer').Buffer as any;
...
export const OUTPUT_QUALITY = 85;
...
export function ensureSharpAvailable() {
  if (!sharp) {
...
export function parseDataUrl(dataUrl: unknown) {
  ...
  const buffer = NodeBuffer.from(String(match[2] || ''), 'base64');
...
export function isRemoteHttpUrl(value: unknown) {
  return /^https?:\/\//i.test(String(value || '').trim());
}

function clearLocalProductImageDir(productId: unknown) {
...
function resolveExistingLocalPath(item: ProductImageInput) {
...
async function optimizeImageBuffer(buffer: unknown) {
...
export async function processProductImages(productId: unknown, images: Array<string | ProductImageInput> = []) {
  ...
  for (const item of normalized.sort((a, b) => Number(a.orden || 0) - Number(b.orden || 0))) {
    ...
    if (remote && isRemoteHttpUrl(remote)) {
      result.push({
        nombre_archivo: item.nombre_archivo || path.basename(String(remote).split('?')[0] || 'imagen-remota'),
        ruta_local: null,
        url_publica: remote,
        url_local: null,
        url_remote: remote,
