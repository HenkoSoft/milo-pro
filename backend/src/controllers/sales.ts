import type { SaleDto, SaleItemDto, SalePayload, SalePayloadItem, SaleStatusUpdatePayload } from '@shared/types/sales';
...
export function normalizeSaleItemPayload(item: unknown): SalePayloadItem {
...
export function normalizeSalePayload(body: unknown): SalePayload {
...
export function normalizeSaleStatusUpdatePayload(body: unknown): SaleStatusUpdatePayload {
  const data = isRecord(body) ? body : {};
  return {
...
export function sanitizeSaleItem(record: unknown): SaleItemDto {
...
export function sanitizeSale(record: unknown, items: SaleItemDto[] = []): SaleDto {
  const data = isRecord(record) ? record : {};
  return {
    id: toNumber(data.id),
