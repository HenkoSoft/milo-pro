import type {
  WooConfigPayload,
  WooConnectionTestPayload,
  WooOrderImportPayload,
  WooOrderSyncLogDto,
  WooPollingResultDto,
  WooPollingStatusDto,
  WooProductSyncLogDto,
  WooStatusResponse
...
function toJsonString<T>(value: unknown, fallback: T): string {
...
function preserveSecret(input: unknown, fallback = ''): string {
  const nextValue = toNullableString(input);
  return nextValue || fallback;
}

export function normalizeWooConnectionTestPayload(body: unknown): WooConnectionTestPayload {
...
export function normalizeWooConfigPayload(body: unknown, existing: unknown): WooConfigPayload {
  const data = isRecord(body) ? body : {};
...
export function normalizeWooOrderImportPayload(body: unknown): WooOrderImportPayload {
  const data = isRecord(body) ? body : {};
...
export function normalizeWooLogsLimit(value: unknown): number {
  return Math.min(Math.max(toNumber(value, 100), 1), 500);
}

export function sanitizeWooStatusResponse(record: unknown): WooStatusResponse {
...
export function sanitizeWooPollingResult(record: unknown): WooPollingResultDto {
  const data = isRecord(record) ? record : {};
  return {
...
export function sanitizeWooPollingStatus(record: unknown): WooPollingStatusDto {
  const data = isRecord(record) ? record : {};
  return {
...
export function sanitizeWooProductSyncLog(record: unknown): WooProductSyncLogDto {
  const data = isRecord(record) ? record : {};
  return {
    id: toNumber(data.id),
...
export function sanitizeWooOrderSyncLog(record: unknown): WooOrderSyncLogDto {
  const data = isRecord(record) ? record : {};
  return {
    id: toNumber(data.id),
