...
type TransactionCallback<T = unknown> = (...args: any[]) => T | Promise<T>;
...
let db: LegacySqliteDb = null as any;
...
function transaction<T = unknown>(fn: TransactionCallback<T>) {
...
function ensureLegacyDatabaseAvailable(methodName: string): void {
  if (POSTGRES_BRIDGE_MODE) {
    throw new Error(
