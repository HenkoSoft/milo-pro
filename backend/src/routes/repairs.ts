import type { RepairDto, RepairLogDto, RepairPayload } from '@shared/types/operations';
...
export function normalizeRepairPayload(body: unknown): RepairPayload {
...
export function sanitizeRepairLog(record: unknown): RepairLogDto {
...
export function sanitizeRepair(record: unknown, statusLabels: Record<string, string>, logs: RepairLogDto[] = []): RepairDto {
  const data = isRecord(record) ? record : {};
