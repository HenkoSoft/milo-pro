...
function buildDashboardStats(record: Record<string, unknown> | null) {
...
function sanitizeLowStockProduct(record: Record<string, unknown> | null) {
  const data = record && typeof record === 'object' ? record : {};
  return {
    ...data,
    id: toNumber(data.id),
...
function sanitizeReadyRepair(record: Record<string, unknown> | null) {
  const data = record && typeof record === 'object' ? record : {};
  return {
    ...data,
    id: toNumber(data.id),
...
function sanitizeActivity(record: Record<string, unknown> | null, type: string) {
  const data = record && typeof record === 'object' ? record : {};
  return {
    ...data,
    id: toNumber(data.id),
...
router.get('/alerts', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  const db = getDatabaseAccess(req);
  const lowStock = (
    await db.all(
      `
...
router.get('/recent-activity', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  const db = getDatabaseAccess(req);
  const recentSales = (
    await db.all(
      `
