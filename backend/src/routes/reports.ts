...
function sanitizeRevenuePoint(record: Record<string, unknown> | null) {
...
router.get('/sales', authenticate, async (req: RouteRequest, res: JsonResponse) => {
...
router.get('/repairs', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  const db = getDatabaseAccess(req);
  const byStatus = (
    await db.all(
      `
...
router.get('/products', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  const db = getDatabaseAccess(req);
  const topSelling = (
    await db.all(
      `
...
router.get('/revenue', authenticate, async (req: RouteRequest, res: JsonResponse) => {
  const db = getDatabaseAccess(req);
