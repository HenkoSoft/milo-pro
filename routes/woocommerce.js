...
const wooPollingManager = createWooPollingManager({
  ...
  markLastSync: () => run('UPDATE woocommerce_sync SET last_sync = CURRENT_TIMESTAMP WHERE id = 1'),
...
async function hydrateWooProductForImport(wooProduct) {
  ...
  try {
    const detailed = await woocommerceRequest('GET', /products/);
